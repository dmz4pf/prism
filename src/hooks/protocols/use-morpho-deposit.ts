/**
 * Morpho Blue Deposit Hook
 *
 * Handles deposits to Morpho Blue curated vaults (ERC-4626 compliant).
 * Features:
...
export interface UseMorphoDepositParams {
...
export type MorphoDepositStep = 'idle' | 'approving' | 'depositing' | 'success' | 'error';

export interface UseMorphoDepositReturn {
  /** Current step in the flow */
  step: MorphoDepositStep;
  ...
  /** Preview shares to receive for amount */
...
export function useMorphoDeposit({
  ...
  assetSymbol,
  decimals = 6,
}: UseMorphoDepositParams): UseMorphoDepositReturn {
  ...
  const approve = useCallback(
    ...
    [assetAddress, vaultAddress, userAddress, decimals, writeApprove]
  );
  ...
  const deposit = useCallback(
    ...
    [vaultAddress, userAddress, decimals, maxDeposit, writeDeposit]
  );
